...
az group create --name "$RG" --location "$LOCATION" \
  --tags "curso=AZ-204" "modulo=M04" "submodulo=S4.2" \
  --output none
...
echo
echo "Endpoints:"
echo "  POST /api/pedidos/procesar              inicia la saga"
echo "  GET  /api/pedidos/estado/{instanceId}   consulta estado"
echo "  POST /api/pedidos/{instanceId}/aprobar  evento humano (slide 9)"
echo "  POST /api/facturas/lote                 fan-out/fan-in (slide 7)"
echo
